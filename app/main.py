...
/* ── Меню на кнопках (пункты навигации) ──────────────────────────────────── */
...
/* Кнопки-поповеры (Руководство, Меню пользователя) — лёгкие пункты меню */
...
/* Отступ у поповеров, чтобы не наезжали на соседние блоки */
section[data-testid="stSidebar"] [data-testid="stPopover"] { margin-top: 16px !important; }
...
/* ── Метрики (новый стиль) ───────────────────────────────────────────────── */
.mc-grid { display:grid; grid-template-columns:repeat(5,minmax(0,1fr)); gap:12px; margin:12px 0 22px; }
.mc { background:#fff; border:1px solid #E2E8F0; border-radius:14px; padding:16px 18px;
      position:relative; overflow:hidden; box-shadow:0 1px 3px rgba(15,23,42,.05);
      transition:transform .12s ease, box-shadow .12s ease; }
.mc:hover { transform:translateY(-2px); box-shadow:0 6px 18px rgba(15,23,42,.09); }
...
.mc-label { font-size:10px; font-weight:700; letter-spacing:.5px; text-transform:uppercase; color:#64748B; }
.mc-ic { font-size:16px; line-height:1; opacity:.9; }
.mc-value { font-size:30px; font-weight:800; color:#0F172A; line-height:1; }
...
/* ── Заголовки секций ────────────────────────────────────────────────────── */
.section-header { font-size:15px; font-weight:700; color:#0F172A; margin:2px 0 10px;
                  display:flex; align-items:center; justify-content:space-between;
                  padding-bottom:6px; border-bottom:1px solid #E5E7EB; }
.section-link   { font-size:12px; font-weight:500; color:#3B82F6; cursor:pointer; }

/* ── Карточки быстрых действий ───────────────────────────────────────────── */
.qa-card { background:#fff; border:1px solid #E2E8F0; border-radius:14px;
           box-shadow:0 1px 3px rgba(15,23,42,.05); padding:14px 16px 8px;
           transition:transform .12s ease, box-shadow .12s ease, border-color .12s ease; }
.qa-card:hover { transform:translateY(-2px); box-shadow:0 6px 18px rgba(15,23,42,.09); border-color:#93C5FD; }
.qa-ic { font-size:20px; margin-bottom:8px; }
.qa-ttl { font-size:14px; font-weight:700; color:#0F172A; }
.qa-desc { font-size:11.5px; color:#64748B; margin-top:3px; line-height:1.35; min-height:32px; }

/* ── Таблицы (единый стиль) ──────────────────────────────────────────────── */
.ipw-table { width:100%; border-collapse:collapse; background:#fff;
             border:1px solid #E2E8F0; border-radius:12px; overflow:hidden; }
.ipw-table thead tr { background:#F8FAFC; }
.ipw-table th { padding:9px 14px; text-align:left; font-size:10px; font-weight:700;
                letter-spacing:.5px; text-transform:uppercase; color:#64748B; }
.ipw-table td { padding:10px 14px; font-size:13px; color:#0F172A; border-top:1px solid #F1F5F9; }
.ipw-table tbody tr:nth-child(even) { background:#FCFDFE; }
.ipw-table tbody tr:hover { background:#F5F9FF; }
...
with st.sidebar:
    ...
    if "_goto_page" in st.session_state:
        _tgt = st.session_state.pop("_goto_page")
        if _tgt in _menu_items:
            st.session_state["current_page"] = _tgt
    page = st.session_state["current_page"]
    if page not in _menu_items:
        page = "🏠 Главная"
        st.session_state["current_page"] = page

    for _gt, _gi in _GROUPS:
        st.markdown(f'<p class="sb-section-label">{_gt}</p>', unsafe_allow_html=True)
        for _it in _gi:
            if st.button(_it, key=f"nav_{_it}", use_container_width=True,
                         type="primary" if _it == page else "secondary"):
                st.session_state["current_page"] = _it
                st.rerun()
...
5. Сформировать отчёт
    ...
    st.markdown(
        ...
        "margin:14px 8px 6px;'></div>",
        unsafe_allow_html=True,
    )
    if _current_user:
        _uname = _auth_name or _current_user.get("email", "")
        _initials = ("".join(w[0] for w in (_auth_name or "?").split()[:2]) or "?").upper()
        _role_lbl = _auth.ROLE_LABELS.get(_current_user.get("role"), _current_user.get("role", ""))
        st.markdown(f"""
        <div style='display:flex;align-items:center;gap:10px;
                    background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.10);
                    border-radius:10px;padding:10px 12px;margin:4px 4px 10px;'>
            <div style='width:36px;height:36px;border-radius:50%;background:#2563EB;color:#fff;
                        display:flex;align-items:center;justify-content:center;
                        font-size:13px;font-weight:700;flex:none;'>{_initials}</div>
            <div style='line-height:1.25;overflow:hidden;'>
                <div style='font-size:13px;font-weight:600;color:#fff;white-space:nowrap;
                            overflow:hidden;text-overflow:ellipsis;'>{_uname}</div>
                <div style='font-size:11px;color:rgba(255,255,255,0.45);'>{_role_lbl}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        with st.popover("⚙️ Меню пользователя", use_container_width=True):
            if st.button("👤 Профиль", use_container_width=True, key="um_profile"):
                st.session_state["current_page"] = "⚙️ Настройки"
                st.rerun()
            if st.button("🔑 Сменить пароль", use_container_width=True, key="um_pwd"):
                st.session_state["current_page"] = "⚙️ Настройки"
                st.rerun()
            if st.button("🚪 Выйти из системы", use_container_width=True, key="um_logout"):
                try:
                    ...
                    pass
                _clear_session_cookie()
                for _k in ("auth_user", "reg_stage", "reg_pending_email",
                           "reg_dev_code", "current_page"):
...
if page == "🏠 Главная":
    ...
    def _parse_dt(v):
        if not v:
            return None
        try:
            dt = datetime.fromisoformat(str(v).replace("Z", "+00:00").replace(" ", "T", 1))
            if dt.tzinfo is None:
                dt = dt.replace(tzinfo=timezone.utc)
    ...
    _now = kz_now()
    _h = _now.hour
    _greet = ("Доброе утро" if 5 <= _h < 12 else "Добрый день" if 12 <= _h < 17
              else "Добрый вечер" if 17 <= _h < 23 else "Добро пожаловать")
    _first = (_auth_name or "").strip().split()[0] if (_auth_name or "").strip() else ""
    _gname = f", {_first}" if _first else ""

    # ── Данные (с учётом владельца) ───────────────────────────────────────────
    _own = _owner_scope()
    _fmw = "" if _own is None else " AND fm.owner_email = ?"
    _fmp = [] if _own is None else [_own]
    _pfw = "" if _own is None else " AND mp.owner_email = ?"
    _pfp = [] if _own is None else [_own]
    _srw = "" if _own is None else (
    ...
    _today = kz_today()
    _dates = [d for d in (_parse_dt(r["ff"]) for r in _ffrows) if d]
    new_today = sum(1 for d in _dates if d.date() == _today)
    new_yest = sum(1 for d in _dates if d.date() == _today - _td(days=1))
    week_new = sum(1 for d in _dates if (_today - d.date()).days < 7)
    _delta_matches = new_today - new_yest

    # ── Приветствие + системные действия ──────────────────────────────────────
    _t1, _t2 = st.columns([3, 1.15])
    with _t1:
        st.markdown(f"""
        <div style="margin-bottom:2px;">
            ...
            <div style="font-size:13.5px;color:#64748B;margin-top:4px;">
                IP Watch KZ — мониторинг товарных знаков и анализ рисков
            </div>
        </div>
        """, unsafe_allow_html=True)
    with _t2:
    ...
    _status_map = {
        "success": ("#16A34A", "Успешно"), "running": ("#2563EB", "Выполняется"),
        "error": ("#DC2626", "Ошибка"), "warning": ("#F59E0B", "С предупреждениями"),
    }
    _sc_color, _sc_label = _status_map.get(last_status, ("#94A3B8", "Не запускалась"))
    _last_str = fmt_datetime_kz(last_dt) if last_dt else "—"
    _delta_html = ""
    if _delta_matches > 0:
        _delta_html = f"<span style='color:#DC2626;font-size:14px;font-weight:600;margin-left:6px;'>↑ {_delta_matches}</span>"
    elif _delta_matches < 0:
        _delta_html = f"<span style='color:#16A34A;font-size:14px;font-weight:600;margin-left:6px;'>↓ {abs(_delta_matches)}</span>"

    _sum_l, _sum_btn = st.columns([3.3, 1])
    with _sum_l:
        st.markdown(f"""
        <div class="card" style="display:flex;gap:34px;align-items:center;padding:16px 22px;">
            <div>
                <div style="font-size:11px;color:#64748B;font-weight:600;text-transform:uppercase;letter-spacing:.4px;">Последняя проверка</div>
                <div style="font-size:16px;font-weight:700;color:#0F172A;margin-top:4px;">
                    <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:{_sc_color};margin-right:7px;vertical-align:middle;"></span>{_last_str}
                </div>
                <div style="font-size:11px;color:{_sc_color};margin-top:2px;">{_sc_label}</div>
            </div>
            <div>
                <div style="font-size:11px;color:#64748B;font-weight:600;text-transform:uppercase;letter-spacing:.4px;">Проверено профилей</div>
                <div style="font-size:24px;font-weight:800;color:#0F172A;margin-top:4px;">{active_profiles}</div>
            </div>
            <div>
                <div style="font-size:11px;color:#64748B;font-weight:600;text-transform:uppercase;letter-spacing:.4px;">Новых совпадений</div>
                <div style="font-size:24px;font-weight:800;color:#0F172A;margin-top:4px;">{new_today}{_delta_html}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    with _sum_btn:
    ...
    with _rp:
        ...
        for r in _dlrows:
            ...
            if ct.get("deadline"):
                ...
                left = (dd - _today).days
                if 0 <= left <= 365:
                    _dls.append((left, dd, r["d"], ct["status"]))
        _dls.sort()
        _dl_html = ""
        _sc = {"urgent": "#DC2626", "open": "#16A34A", "expired": "#94A3B8"}
        for left, dd, name, stt in _dls[:4]:
            clr = _sc.get(stt, "#94A3B8")
            _dl_html += (
                f"<div style='padding:8px 0;border-bottom:1px solid #F1F5F9;'>"
                f"<div style='font-size:13px;font-weight:600;color:#0F172A;'>{name}</div>"
                f"<div style='font-size:11.5px;color:{clr};margin-top:1px;'>Истекает через {left} дн. · {dd:%d.%m.%Y}</div></div>")
        if not _dl_html:
            _dl_html = "<div style='font-size:12px;color:#94A3B8;padding:6px 0;'>Ближайших сроков нет</div>"
        st.markdown(f"""
        <div class="card" style="margin-top:14px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                <div style="font-size:14px;font-weight:700;color:#0F172A;">⏳ Ближайшие сроки</div>
                <div style="font-size:11px;color:#2563EB;">{len(_dls)}</div>
            </div>
            {_dl_html}
        </div>
        """, unsafe_allow_html=True)
    ...
    with _main:
        # Быстрые действия
        st.markdown('<div class="section-header">Быстрые действия</div>', unsafe_allow_html=True)
        _qa = [
            ("📁", "Новый профиль", "Добавить профиль для мониторинга", "📋 Профили мониторинга"),
            ("🔎", "Единый поиск", "Поиск по товарным знакам и заявкам", "🔍 Единый поиск"),
        ...
        for i, (ic, ttl, desc, target) in enumerate(_qa):
            with _qc[i]:
                st.markdown(f"""
                <div class="qa-card">
                    <div class="qa-ic">{ic}</div>
                    <div class="qa-ttl">{ttl}</div>
                    <div class="qa-desc">{desc}</div>
                </div>
                """, unsafe_allow_html=True)
                if st.button("Открыть", key=f"qa_{i}", use_container_width=True):
                    _goto(target)
        ...
        <div class="mc-grid" style="grid-template-columns:repeat(5,minmax(0,1fr));">
            ...
            <div class="mc mc-slate">
                <div class="mc-top"><span class="mc-label">Всего найдено</span><span class="mc-ic">🔍</span></div>
                <div class="mc-value">{total_marks}</div>
                <div class="mc-sub">↑ {week_new} за неделю</div>
            </div>
            <div class="mc mc-red">
                <div class="mc-top"><span class="mc-label">Высокий риск</span><span class="mc-ic">🚨</span></div>
                <div class="mc-value">{high_risk}</div>
                <div class="mc-sub">Требует внимания</div>
            </div>
            <div class="mc mc-amber">
                <div class="mc-top"><span class="mc-label">Средний риск</span><span class="mc-ic">⚠️</span></div>
                <div class="mc-value">{medium_risk}</div>
                <div class="mc-sub">Правовая оценка</div>
            </div>
            <div class="mc mc-green">
                <div class="mc-top"><span class="mc-label">Низкий риск</span><span class="mc-ic">✅</span></div>
                <div class="mc-value">{low_risk}</div>
                <div class="mc-sub">Без срочных действий</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
...
# ─── СТАТЬИ И ОБЗОРЫ (в разработке) ───────────────────────────────────────────

elif page == "📄 Статьи и обзоры":
    st.title("Статьи и обзоры")
    st.info("📄 Раздел в разработке. Здесь появятся аналитические статьи, обзоры практики "
            "и разъяснения по товарным знакам и интеллектуальной собственности.")


# ─── СПРАВОЧНИКИ (в разработке) ───────────────────────────────────────────────

elif page == "📗 Справочники":
    st.title("Справочники")
    st.info("📗 Раздел в разработке. Здесь появятся справочники: классы МКТУ, коды стран, "
            "статусы заявок, типовые основания для возражений.")
